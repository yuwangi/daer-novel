...
function NovelDetail() {
  ...
  // 防重复点击 - 使用 ref 进行同步检查
  const isGeneratingOutlineRef = useRef(false);

  const handleGenerateOutlineStream = (mode: string) => {
    // 防重复点击检查 - 使用 ref 进行同步检查
    if (isGeneratingOutlineRef.current) {
      toast.info("大纲生成任务正在进行中，请勿重复点击");
      return;
    }

    isGeneratingOutlineRef.current = true;
    setIsStreaming(true);
    if (mode === "initial") {
      setGeneratedOutline("");
    }
    ...
    eventSource.onmessage = (event) => {
      ...
      if (data.type === "chunk") {
        ...
        // 延迟重置 ref，确保状态更新完成
        setTimeout(() => {
          isGeneratingOutlineRef.current = false;
        }, 500);
