...
const ChapterGenerator = dynamic(
  () => import("@/components/novel/ChapterGenerator"),
  { loading: () => <TabSkeleton /> },
);
const KnowledgeManager = dynamic(
  () => import("@/components/novel/KnowledgeManager"),
  { loading: () => <TabSkeleton /> },
);
const CharacterManager = dynamic(
  () => import("@/components/novel/CharacterManager"),
  { loading: () => <TabSkeleton /> },
);
const PlotThreadManager = dynamic(
  () => import("@/components/novel/PlotThreadManager"),
  { loading: () => <TabSkeleton /> },
);
const TimelineManager = dynamic(
  () => import("@/components/novel/TimelineManager"),
  { loading: () => <TabSkeleton /> },
);
...
function NovelDetail() {
  ...
  // Outline view mode
  type OutlineViewMode = "readable" | "raw";
  const [outlineViewMode, setOutlineViewMode] =
    useState<OutlineViewMode>("readable");

  // Helper: get display content based on mode
  const getOutlineDisplayContent = (
    version: any,
    mode: OutlineViewMode,
  ): string => {
    if (!version) return "";
    if (mode === "readable" && version.isStructured && version.displayContent) {
      return version.displayContent;
    }
  ...
  const handleGenerateOutlineStream = (mode: string) => {
    ...
    const eventSource = novelsAPI.generateOutlineStream(
      novelId,
      mode,
      existingOutline,
    );
    ...
    eventSource.onmessage = (event) => {
      ...
      if (data.type === "chunk") {
        ...
        setGeneratedOutline((prev) =>
          mode === "initial" ? prev + data.content : prev + data.content,
        ); // Simplified logic
        // Auto scroll
    ...
    try {
      await novelsAPI.saveOutlineVersion(novelId, {
        ...
        context: {
          ...
          knowledgeBases:
            displayNovel?.knowledgeBases?.map((k: any) => k.name) || [],
          mode,
        },
      });
  ...
  const handleLock = async (version: any) => {
    try {
      await novelsAPI.lockOutlineVersion(
        novelId,
        version.id,
        !version.isLocked,
      );
  ...
  useEffect(() => {
    if (novelData) setLocalNovel(null);
  }, [novelData]);
  ...
  // Update displayed content when view mode changes
  useEffect(() => {
    if (currentVersion && !isStreaming) {
      const content = getOutlineDisplayContent(currentVersion, outlineViewMode);
      setGeneratedOutline(content);
    }
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [outlineViewMode, currentVersion?.id]);
          ...
          {activeTab === "settings" && (
            <Card className="animate-fadeIn p-6 md:p-8">
              <div className="flex items-center justify-between mb-8 pb-4 border-b border-gray-100 dark:border-gray-800">
                ...
                <Button variant="default" onClick={handleUpdateNovel}>
                  保存修改
                </Button>
              </div>
                    ...
                    <div>
                      ...
                      <div className="flex flex-wrap gap-2 p-4 glass rounded-xl border border-gray-100 dark:border-gray-800">
                        ...
                        {novel.genre?.map((g: string) => (
                          <span
                            key={g}
                            className="px-3 py-1.5 text-sm font-medium rounded-lg bg-primary-50 dark:bg-primary-900/40 text-primary-700 dark:text-primary-300 border border-primary-100 dark:border-primary-800"
                          >
                            {g}
                          </span>
                        ))}
                        {novel.style?.map((s: string) => (
                          <span
                            key={s}
                            className="px-3 py-1.5 text-sm font-medium rounded-lg bg-purple-50 dark:bg-purple-900/40 text-purple-700 dark:text-purple-300 border border-purple-100 dark:border-purple-800"
                          >
                            {s}
                          </span>
                        ))}
                        ...
                        <div className="grid grid-cols-2 gap-4">
                          <div>
                            ...
                            <div className="w-full px-3 py-2.5 bg-gray-50 dark:bg-white/5 rounded-lg text-sm text-gray-700 dark:text-gray-300 font-mono">
                              {novel.targetWords?.toLocaleString() || "未设定"}{" "}
                              字
                            </div>
                          </div>
                          <div>
                            ...
                            <div className="w-full px-3 py-2.5 bg-gray-50 dark:bg-white/5 rounded-lg text-sm text-gray-700 dark:text-gray-300 font-mono">
                              {novel.minChapterWords?.toLocaleString() ||
                                "3000"}{" "}
                              字
                            </div>
                          </div>
                      ...
                      <div className="flex flex-wrap gap-2">
                        {novel.knowledgeBases?.length > 0 ? (
                          novel.knowledgeBases.map((kb: any) => (
                            <span
                              key={kb.id}
                              className="px-3 py-1.5 text-xs font-medium rounded-lg bg-blue-50 dark:bg-blue-900/40 text-blue-700 dark:text-blue-300 border border-blue-100 dark:border-blue-800"
                            >
                              {kb.name}
                            </span>
                          ))
                  ...
                  <div className="space-y-6">
                              ...
                              <img
                                src={
                                  novel.coverUrl.startsWith("/")
                                    ? `${process.env.NEXT_PUBLIC_API_URL || "http://localhost:8002"}${novel.coverUrl}`
                                    : novel.coverUrl
                                }
                                alt="Cover"
                    ...
                    <div>
                      ...
                      <textarea
                        ...
                        onChange={(e) =>
                          setNovel({ ...novel, background: e.target.value })
                        }
              ...
              <div className="grid md:grid-cols-2 gap-8">
                  ...
                  <div className="flex justify-end">
                    <Button
                      onClick={async () => {
                        ...
                        try {
                          const res = await novelsAPI.extractStyle(
                            novelId,
                            styleSample,
                          );
                          setNovel({
                            ...novel,
                            writingStyleRules: res.data.writingStyleRules,
                          });
                          toast.success("文风提取成功！");
                        } catch (error: any) {
                          toast.error(
                            error.response?.data?.error || "提取失败",
                          );
                        } finally {
                ...
                <div className="space-y-4">
                  ...
                  <textarea
                    ...
                    onChange={(e) =>
                      setNovel({ ...novel, writingStyleRules: e.target.value })
                    }
          ...
          {activeTab === "outline" && (
            <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 animate-fadeIn">
              ...
              >
                  ...
                  <div>
                    ...
                    <p className="text-sm text-gray-500 mt-1 flex items-center gap-2">
                      ...
                      {currentVersion?.isLocked === 1 && (
                        <span className="text-amber-500">(已锁定)</span>
                      )}
                  ...
                  <div className="flex items-center space-x-3">
                    {currentVersion?.isStructured && (
                      <div className="flex items-center bg-gray-100 dark:bg-gray-800 rounded-lg p-1">
                        <button
                          onClick={() => setOutlineViewMode("readable")}
                          className={cn(
                        ...
                        <button
                          onClick={() => setOutlineViewMode("raw")}
                          className={cn(
                            "flex items-center space-x-1.5 px-3 py-1.5 rounded-md text-xs font-medium transition-all",
                            outlineViewMode === "raw"
                              ? "bg-white dark:bg-gray-700 shadow-sm text-primary-600 dark:text-primary-400"
                ...
                {/* Stats Display for Structured Outline */}
                {currentVersion?.isStructured &&
                  currentVersion?.outlineStats && (
                    <div className="mb-4 grid grid-cols-2 sm:grid-cols-4 gap-3">
                      <div className="flex flex-col items-center justify-center p-3 rounded-lg bg-blue-50 dark:bg-blue-900/20 border border-blue-100 dark:border-blue-800/50">
                        <span className="text-2xl font-bold text-blue-600 dark:text-blue-400">
                          {currentVersion.outlineStats.phaseCount || 0}
                        </span>
                        <span className="text-xs text-gray-500 dark:text-gray-400 mt-1">
                          阶段数
                        </span>
                      </div>
                      <div className="flex flex-col items-center justify-center p-3 rounded-lg bg-emerald-50 dark:bg-emerald-900/20 border border-emerald-100 dark:border-emerald-800/50">
                        <span className="text-2xl font-bold text-emerald-600 dark:text-emerald-400">
                          {currentVersion.outlineStats.totalEvents || 0}
                        </span>
                        <span className="text-xs text-gray-500 dark:text-gray-400 mt-1">
                          事件数
                        </span>
                      </div>
                      <div className="flex flex-col items-center justify-center p-3 rounded-lg bg-purple-50 dark:bg-purple-900/20 border border-purple-100 dark:border-purple-800/50">
                        <span className="text-2xl font-bold text-purple-600 dark:text-purple-400">
                          {currentVersion.outlineStats.chapterCountEstimate
                            ? currentVersion.outlineStats.chapterCountEstimate.toLocaleString()
                            : "—"}
                        </span>
                        <span className="text-xs text-gray-500 dark:text-gray-400 mt-1">
                          预计章节
                      ...
                      <div className="flex flex-col items-center justify-center p-3 rounded-lg bg-amber-50 dark:bg-amber-900/20 border border-amber-100 dark:border-amber-800/50">
                        <span className="text-2xl font-bold text-amber-600 dark:text-amber-400">
                          {currentVersion.outlineStats.cliffhangerCount || 0}
                        </span>
                        <span className="text-xs text-gray-500 dark:text-gray-400 mt-1">
                          悬念点
                        </span>
                    ...
                    !isStreaming && (
                      <div className="mb-3 p-3 rounded-lg bg-amber-50 dark:bg-amber-900/20 border border-amber-200 dark:border-amber-800/50">
                        <p className="text-xs text-amber-700 dark:text-amber-300 flex items-start gap-2">
                          ...
                          <span>
                            当前为<strong>可读视图</strong>
                            ，编辑后保存会将内容转换为纯文本格式（丢失结构化数据的统计和视图切换功能）。
                            如需保持结构化大纲，请切换到{" "}
                            <strong>「原始 JSON」</strong> 视图进行编辑。
                          </span>
                        </p>
                      </div>
                    )}
                    ...
                    <div className="flex gap-2">
                      <Button
                        ...
                        onClick={async () => {
                          ...
                          try {
                            const res = await sandboxAPI.create(novelId, {
                              title: sandboxTitle,
                              premise: sandboxPremise,
                            });
                      ...
                      <Button
                        size="sm"
                        variant="ghost"
                        onClick={() => setIsCreatingSandbox(false)}
                      >
                        取消
                      </Button>
                    </div>
                  ...
                  <>
                      ...
                      <Button
                        ...
                        onClick={async () => {
                          ...
                          try {
                            ...
                            setSandboxes((prev) =>
                              prev.filter((s) => s.id !== activeSandbox.id),
                            );
                            setActiveSandbox(null);
                            toast.success("已删除");
                          } catch {
                    ...
                    <div>
                      ...
                      <textarea
                        ...
                        onBlur={async () => {
                          try {
                            await sandboxAPI.update(activeSandbox.id, {
                              premise: activeSandbox.premise,
                            });
                          } catch {}
                        }}
                      ...
                      <Button
                        onClick={async () => {
                          ...
                          try {
                            ...
                            const res = await sandboxAPI.generate(
                              activeSandbox.id,
                            );
                            ...
                            toast.error(
                              err?.response?.data?.error || "推演生成失败",
                            );
                          } finally {
          ...
          <div className="space-y-4">
            <p className="text-gray-600 dark:text-gray-300">
              ...
              {versionToRollback?.version
                ? `v${versionToRollback.version}`
                : "该版本"}{" "}
              吗？这将基于该版本创建一个最新的大纲版本。
            </p>
