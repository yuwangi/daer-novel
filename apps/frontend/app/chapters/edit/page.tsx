...
function ChapterEditor() {
  ...
  // Toggle Focus Mode - uses functional update to ensure consistent state
  const toggleFocusMode = useCallback(() => {
    setIsFocusMode((prevMode) => {
      const newMode = !prevMode;
      // Auto-close panels when entering focus mode, restore when exiting
      if (newMode) {
        // Entering focus mode - close all panels
        setIsLeftSidebarOpen(false);
        setIsRightPanelOpen(false);
        setIsHistoryOpen(false);
      } else {
        // Exiting focus mode - restore default panels
  ...
  const handleRewrite = async (text: string, prompt?: string) => {
    if (!isRightPanelOpen) setIsRightPanelOpen(true);

    // Auto-create snapshot before AI rewrite (fire and forget)
    if (chapterId) {
      try {
        await tasksAPI.createSnapshot(chapterId, "AI改写前自动备份");
      } catch (e) {
        // Silently fail - snapshot creation should not block rewrite
        console.warn("Failed to create pre-rewrite snapshot:", e);
      }
    }
  ...
  const handleExpand = async (text: string) => {
    ...
    // Auto-create snapshot before AI expand (fire and forget)
    if (chapterId) {
      try {
        await tasksAPI.createSnapshot(chapterId, "AI扩写前自动备份");
      } catch (e) {
        // Silently fail - snapshot creation should not block expand
        console.warn("Failed to create pre-expand snapshot:", e);
      }
    }
