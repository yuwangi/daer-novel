...
const detectChaptersRegex = (
  content: string,
): { title: string; order: number; content: string }[] => {
  const chapterRegex =
    /(第[零一二三四五六七八九十百千万\d]+[章节回部卷]|[Cc]hapter\s*\d+|^\d+[\.\s、])\s*([^\n\r]{1,100})/gm;
...
router.post("/:id/extract-style", async (req: AuthRequest, res, next) => {
  try {
    ...
    const styleRulesResponse = await styleAgent.execute(
      { novel: novel as any },
      sampleText,
    );
  ...
  async (req: AuthRequest, res, next) => {
    ...
    try {
      ...
      if (isUpstreamPrematureClose) {
        if (!clientClosed) {
          res.write(
            `data: ${JSON.stringify({ type: "error", error: "AI连接中断，请重试" })}\n\n`,
          );
          res.end();
        }
        return;
...
// Lock/Unlock version
router.patch(
  "/:novelId/outline/versions/:versionId/lock",
  async (req: AuthRequest, res, next) => {
    try {
      const { outlineService } = await import("../services/outline.service");
      const { isLocked } = req.body;
      const [version] = await outlineService.toggleLock(
...
// Rollback version
router.post(
  "/:id/outline/versions/:versionId/rollback",
  async (req: AuthRequest, res, next) => {
    try {
      const { outlineService } = await import("../services/outline.service");
      const version = await outlineService.rollback(
        req.params.id,
        req.params.versionId,
      );
      res.status(201).json(version);
...
router.post("/suggestions/titles", async (req: AuthRequest, res, next) => {
  try {
    ...
    console.log(
      `[AI] Title suggestion requested for genre: ${genre}, style: ${style}`,
    );

    const provider = await createAIProvider(req.userId!);
    const config = (provider as any).config;
    console.log(
      `[AI] Using provider: ${config?.provider}, model: ${config?.model}, baseUrl: ${config?.baseUrl || "default"}`,
    );
...
router.get("/:id/export", async (req: AuthRequest, res, next) => {
  try {
    ...
    if (format === "txt") {
      ...
      res.setHeader(
        "Content-Disposition",
        `attachment; filename="${encodeURIComponent(novel.title || "novel")}.txt"`,
      );
      res.send(txtContent);
      return;
    }
    ...
    if (format === "epub") {
      ...
      const content = novel.volumes.flatMap((v) =>
        v.chapters.map((ch) => ({
          title:
            novel.volumes.length > 1 ? `${v.title} - ${ch.title}` : ch.title,
      ...
      res.setHeader(
        "Content-Disposition",
        `attachment; filename="${encodeURIComponent(novel.title || "novel")}.epub"`,
      );
      res.send(buffer);
      return;
    }
    ...
    try {
      ...
      if (req.file) {
        ...
        if (ext === ".json") {
          ...
          const title = fixTitleEncoding(
            (epub as any).metadata.title ||
              path.basename(req.file.originalname, ".epub"),
          );
          let fullContent = "";
      ...
      if (initialize) {
        ...
        if (sourceChunks && sourceChunks.length > 0) {
          ...
          const analysisResponse = await globalAgent.execute({
            novel: { ...novelData },
            sourceChunks,
          });
          try {
            ...
            if (Array.isArray(generatedCharacters)) {
              ...
              for (const char of generatedCharacters) {
                await db.insert(schema.characters).values({
                  ...
                  abilities: char.capabilities
                    ? [{ name: "核心能力", level: char.importance || 5 }]
                    : [],
                });
              }
            }
        ...
        if (!volumes || volumes.length === 0) {
          if (detectedChapters && detectedChapters.length > 0) {
            console.log(
              `[Import] Mapping ${detectedChapters.length} detected chapters`,
            );
            ...
            if (detectedChapters.length > 100) {
              ...
              const volumeCount = Math.ceil(
                detectedChapters.length / volumeSize,
              );
                ...
                for (const v of planning.volumes) {
                  const [newVolume] = await db
                    ...
                    .values({
                      novelId: novel.id,
                      title: v.title,
                      summary: v.summary, // Added summary field
        ...
        if (!knowledgeBases || knowledgeBases.length === 0) {
          ...
          const kbResponse = await kbAgent.execute({
            novel: { ...novel, background: currentBackground },
            sourceChunks,
          });

          try {
            const extractedDocs = JSON.parse(
              kbResponse.content
                .replace(/```json/g, "")
                .replace(/```/g, "")
                .trim(),
            ...
            if (kb.documents) {
              for (const doc of kb.documents) {
                await db
                  ...
                  .values({ knowledgeBaseId: newKb.id, ...doc });
              }
            }
          }
        }
        ...
        if (volumes) {
          for (const v of volumes) {
            const [newVolume] = await db
              .insert(schema.volumes)
              .values({ novelId: novel.id, title: v.title, order: v.order })
              .returning();
            if (v.chapters) {
              for (const ch of v.chapters) {
                await db
                  .insert(schema.chapters)
                  .values({ novelId: novel.id, volumeId: newVolume.id, ...ch });
              }
            }
          }
        }
      ...
      if (req.file?.path) {
        fs.unlink(req.file.path, (err) => {
          if (err)
            logger.error("Failed to delete novel import temp file:", err);
        });
      }
    } catch (error) {
      if (req.file?.path) {
