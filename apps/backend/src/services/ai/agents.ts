...
export class VolumePlanningAgent extends BaseAgent {
  ...
  protected buildSystemPrompt(context: AgentContext): string {
    const targetWords = context.novel.targetWords || 1000000;
    const minChapterWords = context.novel.minChapterWords || 3000;
    const estimatedTotalChapters = Math.ceil(targetWords / minChapterWords);
    const chaptersPerVolume = Math.min(
      50,
      Math.max(20, Math.ceil(estimatedTotalChapters / 5)),
    );
...
小说标题：${context.novel.title}
目标总字数：${targetWords}字
预计总章节数：约 ${estimatedTotalChapters} 章（每章约 ${minChapterWords} 字）
分卷策略：全书分为 4-6 卷，每卷约 ${chaptersPerVolume} 章（20-50 章）

【核心规划原则】：
1. **起承转合结构**：每卷必须遵循完整的"起承转合"节奏：
   - 起：卷的开端，建立新场景/新冲突，引入关键人物或事件
   - 承：情节推进，矛盾升级，关系深化
   - 转：重大转折，形势逆转，真相揭露
   - 合：卷末高潮，部分悬念解开，为下卷留下钩子

2. **情节推进逻辑**：
   - 每卷必须有一个【核心矛盾线】贯穿始终
   - 每卷要有明确的【卷目标】：主角要达成什么？要解决什么危机？
   - 卷与卷之间要有【递进关系】：难度升级、格局扩大、 stakes 提高

3. **节奏把控**：
   - 卷内节奏要有张有弛，不能一直紧张也不能一直平淡
   - 每 3-5 章设置一个小高潮
   - 卷末必须有【大高潮】和【钩子】，吸引读者继续阅读
...
【具体要求】：
1. **宏观拆分**：根据大纲脉络，将全书拆分为 4-6 大卷，卷名要体现该卷的核心主题或关键事件（如：第一卷：初入江湖；第二卷：锋芒初露；第三卷：风云际会...）。
2. **卷概要详尽**：每卷必须提供清晰的剧情走向概要（200-500字），需涵盖：
   - 本卷核心矛盾是什么？
   - 主角的目标是什么？
   - 关键事件节点有哪些？
   - 卷末高潮和钩子是什么？
   - 主角在本卷获得了什么成长？
3. **结构合理**：确保各卷之间逻辑衔接紧密，符合长篇连载的节奏。
...
输出格式（JSON，严禁包含 markdown 代码块）：
{
  "volumes": [
    {
      "title": "卷名",
      "summary": "本卷核心剧情概要（200-500字，需包含核心矛盾、主角目标、关键节点、卷末钩子等）"
    }
  ]
}`;
  }
...
1. **情节推进逻辑（关键！）**：
   - 每章必须有明确的【情节推进目标】：这一章要发生什么？要推动什么剧情？
   - 章节之间要有【因果关系】：上一章的结尾应该是下一章的开端或伏笔
   - 避免"流水账"：每章都要有冲突、转折或发现，不能只是平铺直叙的日常

2. **起承转合在章节层面的应用**：
   - 【起】：章节开头要快速切入，或承接上一章的钩子
   - 【承】：中间部分推进情节，深化冲突，展示人物互动
   - 【转】：每个章节最好有一个小转折或意外
   - 【合】：章节结尾必须留【钩子】（悬念、问题、意外事件），吸引读者继续阅读

3. **节奏把控策略**：
   - 卷的前 1/4：铺垫期，建立场景、人物关系、核心矛盾
   - 卷的中间 2/4：发展期，冲突升级，情节加速，小高潮不断
   - 卷的后 1/4：收束期，所有线索汇聚，走向卷末大高潮
   - 每 3-5 章设置一个【中等高潮】，保持阅读张力
...
4. **章节类型搭配**：
   ...
   - 【过渡章】：场景转换、时间跳跃、信息整理
   - 【高潮章】：重大对决、真相揭露、命运转折
   - 建议：动作章和文戏章交替搭配，避免连续多章同一种类型

5. **钩子设置技巧**：
   - 每章结尾都要有钩子，类型包括：
     - 悬念型："他缓缓转过身，露出了那张不可能出现在这里的脸..."
     - 危机型："就在这时，门外传来了急促的脚步声..."
     - 意外型："他万万没有想到，自己寻找多年的真相竟然是..."
     - 选择型："现在，他必须在两个都无法放弃的选项中做出抉择..."

6. **人物成长线**：
   - 主角在每章都应该有【小的成长】或【认知变化】
   - 配角也要有戏份，不能只是工具人
   - 人物关系要有发展：从陌生到熟悉、从敌对到合作、从信任到背叛等

【具体要求】：
1. **微观拆分**：根据提供的【卷概要】，将其细化为具体的章节列表。
2. **章节连贯**：每章有明确的主题，概要需详尽（80字以上），需包含：
   - 本章的核心冲突是什么？
   - 有哪些关键人物出场？
   - 本章的转折点或钩子是什么？
   - 对整体剧情有什么推进作用？
3. **标题质量**：章节标题要精炼、有吸引力，能体现本章核心内容或悬念。

输出格式（JSON，严禁包含 markdown 代码块）：
{
  "chapters": [
    {"title": "章节标题", "summary": "章节概要（80字以上，需包含核心冲突、关键人物、转折点/钩子、剧情推进作用）"}
  ]
}
`;
  }
  ...
  protected buildUserPrompt(
    ...
    const isContinuing = !!input.existingChaptersContext;
...
${isContinuing ? `【重要：本卷已有章节（必须紧密承接这些剧情继续往下发展！切勿从头开始！）】：\n${input.existingChaptersContext}\n` : ""}

【目标生成章节数】：${input.targetCount} 章

${input.additionalRequirements ? `用户额外要求：\n${input.additionalRequirements}\n` : ""}

【规划策略提示】：
${
  isContinuing
    ? `
1. 【承接要求】：请先仔细阅读上面的【本卷已有章节】，分析：
   - 最后一章的结尾钩子是什么？
   - 当前的主要冲突发展到了什么阶段？
   - 人物关系处于什么状态？
   新章节必须【紧密承接】这些线索，不能跳脱或重启！

2. 【推进要求】：继续推进卷的核心矛盾线，保持与已有章节一致的节奏和风格。
`
    : `
1. 【开端要求】：卷的前几章需要：
   - 快速建立本卷的核心场景和人物状态
...
2. 【结构要求】：按照"铺垫-发展-高潮"的节奏规划全卷章节。
`
}

3. 【钩子要求】：每章结尾必须设置钩子，确保阅读连贯性！
4. 【节奏要求】：动作章和文戏章交替搭配，避免单调。

【重要要求】：
1. 请为本卷生成详细的章节列表，数量应接近 ${input.targetCount} 章。
2. 章节标题必须纯净，**绝对不要**包含任何"第X章"或"第一章"之类的序号前缀。直接输出纯标题内容（例如不要输出"第一章：青铜裂镜"，直接输出"青铜裂镜"）。
3. 章节概要必须详尽（80字以上），要说明本章的核心冲突、关键人物、转折点/钩子、以及对整体剧情的推进作用。
