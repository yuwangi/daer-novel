...
export const characters = pgTable("characters", {
  ...
  relationships:
    jsonb("relationships").$type<{ characterId: string; relation: string }[]>(),
...
export const chapters = pgTable("chapters", {
  ...
  createdAt: timestamp("created_at").defaultNow().notNull(),
  updatedAt: timestamp("updated_at").defaultNow().notNull(),
  contentModifiedAt: timestamp("content_modified_at"), // Track when actual writing content changes (for daily word count stats)
});
...
export const outlineVersionsRelations = relations(
  outlineVersions,
  ({ one }) => ({
    novel: one(novels, {
